...
public class ObjconqcontJpaController implements Serializable {
    ...
    public void create(Objconqcont objconqcont) throws IllegalOrphanException,
	...
	if (objetivoOrphanCheck != null) {
	    Objconqcont oldObjconqcontOfObjetivo = objetivoOrphanCheck
		    .getObjconqcont();
		...
		illegalOrphanMessages
			.add("The Objetivo "
				+ objetivoOrphanCheck
				+ " already has an item of type Objconqcont whose objetivo column cannot be null. Please make another selection for the objetivo field.");
	    }
	}
	if (illegalOrphanMessages != null) {
	...
	try {
	    ...
	    for (Continente continenteCollectionContinenteToAttach : objconqcont
	    ...
	    for (Continente continenteCollectionContinente : objconqcont
		    .getContinenteCollection()) {
		continenteCollectionContinente.getObjconqcontCollection().add(
			objconqcont);
		...
		throw new PreexistingEntityException("Objconqcont "
			+ objconqcont + " already exists.", ex);
	    }
	    throw ex;
	} finally {
	    if (em != null) {
    ...
    public void edit(Objconqcont objconqcont) throws IllegalOrphanException,
	    NonexistentEntityException, Exception {
	EntityManager em = null;
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Objconqcont persistentObjconqcont = em.find(Objconqcont.class,
	    ...
	    Collection<Continente> continenteCollectionOld = persistentObjconqcont
		    .getContinenteCollection();
	    Collection<Continente> continenteCollectionNew = objconqcont
		    .getContinenteCollection();
	    List<String> illegalOrphanMessages = null;
	    if (objetivoNew != null && !objetivoNew.equals(objetivoOld)) {
		Objconqcont oldObjconqcontOfObjetivo = objetivoNew
			.getObjconqcont();
		if (oldObjconqcontOfObjetivo != null) {
		    if (illegalOrphanMessages == null) {
			illegalOrphanMessages = new ArrayList<String>();
		...
		if (!continenteCollectionNew
		    ...
		    continenteCollectionOldContinente
			    .getObjconqcontCollection().remove(objconqcont);
		    continenteCollectionOldContinente = em
			    .merge(continenteCollectionOldContinente);
		}
	    }
		...
		if (!continenteCollectionOld
		    ...
		    continenteCollectionNewContinente
			    .getObjconqcontCollection().add(objconqcont);
	...
	try {
		...
		throw new NonexistentEntityException("The objconqcont with id "
	    ...
	    Collection<Continente> continenteCollection = objconqcont
		    .getContinenteCollection();
	    for (Continente continenteCollectionContinente : continenteCollection) {
		continenteCollectionContinente.getObjconqcontCollection()
			.remove(objconqcont);
		continenteCollectionContinente = em
			.merge(continenteCollectionContinente);
	    }
    ...
    public List<Objconqcont> findObjconqcontEntities(int maxResults,
	    int firstResult) {
	return findObjconqcontEntities(false, maxResults, firstResult);
    }

    private List<Objconqcont> findObjconqcontEntities(boolean all,
