...
public class ContinenteJpaController implements Serializable {
	...
	try {
	    ...
	    for (Objconqcont objconqcontCollectionObjconqcontToAttach : continente
		    .getObjconqcontCollection()) {
		objconqcontCollectionObjconqcontToAttach = em.getReference(
			objconqcontCollectionObjconqcontToAttach.getClass(),
			objconqcontCollectionObjconqcontToAttach
				.getCodObjetivo());
		attachedObjconqcontCollection
			.add(objconqcontCollectionObjconqcontToAttach);
	    }
	    ...
	    for (Territorio territorioCollectionTerritorioToAttach : continente
	    ...
	    for (Objconqcont objconqcontCollectionObjconqcont : continente
		    .getObjconqcontCollection()) {
		objconqcontCollectionObjconqcont.getContinenteCollection().add(
			continente);
		objconqcontCollectionObjconqcont = em
			.merge(objconqcontCollectionObjconqcont);
	    }
	    for (Territorio territorioCollectionTerritorio : continente
		    .getTerritorioCollection()) {
		Continente oldCodContinenteOfTerritorioCollectionTerritorio = territorioCollectionTerritorio
			.getCodContinente();
		...
		if (oldCodContinenteOfTerritorioCollectionTerritorio != null) {
		    oldCodContinenteOfTerritorioCollectionTerritorio
			    .getTerritorioCollection().remove(
				    territorioCollectionTerritorio);
		    oldCodContinenteOfTerritorioCollectionTerritorio = em
			    .merge(oldCodContinenteOfTerritorioCollectionTerritorio);
		}
	    }
    ...
    public void edit(Continente continente) throws IllegalOrphanException,
	    NonexistentEntityException, Exception {
	EntityManager em = null;
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Continente persistentContinente = em.find(Continente.class,
		    continente.getCodContinente());
	    Mundo codMundoOld = persistentContinente.getCodMundo();
	    Mundo codMundoNew = continente.getCodMundo();
	    Collection<Objconqcont> objconqcontCollectionOld = persistentContinente
		    .getObjconqcontCollection();
	    Collection<Objconqcont> objconqcontCollectionNew = continente
		    .getObjconqcontCollection();
	    Collection<Territorio> territorioCollectionOld = persistentContinente
		    .getTerritorioCollection();
	    Collection<Territorio> territorioCollectionNew = continente
		    .getTerritorioCollection();
		...
		if (!territorioCollectionNew
		    ...
		    illegalOrphanMessages
			    .add("You must retain Territorio "
				    + territorioCollectionOldTerritorio
				    + " since its codContinente field is not nullable.");
		}
	    }
	    if (illegalOrphanMessages != null) {
		...
		objconqcontCollectionNewObjconqcontToAttach = em.getReference(
			objconqcontCollectionNewObjconqcontToAttach.getClass(),
			objconqcontCollectionNewObjconqcontToAttach
				.getCodObjetivo());
		attachedObjconqcontCollectionNew
			.add(objconqcontCollectionNewObjconqcontToAttach);
	    }
		...
		if (!objconqcontCollectionNew
			.contains(objconqcontCollectionOldObjconqcont)) {
		    objconqcontCollectionOldObjconqcont
			    .getContinenteCollection().remove(continente);
		    objconqcontCollectionOldObjconqcont = em
			    .merge(objconqcontCollectionOldObjconqcont);
		}
	    }
	    for (Objconqcont objconqcontCollectionNewObjconqcont : objconqcontCollectionNew) {
		if (!objconqcontCollectionOld
			.contains(objconqcontCollectionNewObjconqcont)) {
		    objconqcontCollectionNewObjconqcont
			    .getContinenteCollection().add(continente);
		    objconqcontCollectionNewObjconqcont = em
			    .merge(objconqcontCollectionNewObjconqcont);
		}
	    }
		...
		if (!territorioCollectionOld
		    ...
		    Continente oldCodContinenteOfTerritorioCollectionNewTerritorio = territorioCollectionNewTerritorio
			    .getCodContinente();
		    territorioCollectionNewTerritorio
			    .setCodContinente(continente);
		    territorioCollectionNewTerritorio = em
			    .merge(territorioCollectionNewTerritorio);
		    if (oldCodContinenteOfTerritorioCollectionNewTerritorio != null
			    && !oldCodContinenteOfTerritorioCollectionNewTerritorio
				    .equals(continente)) {
			oldCodContinenteOfTerritorioCollectionNewTerritorio
				.getTerritorioCollection().remove(
					territorioCollectionNewTerritorio);
			oldCodContinenteOfTerritorioCollectionNewTerritorio = em
				.merge(oldCodContinenteOfTerritorioCollectionNewTerritorio);
		    }
		}
	    }
	    em.getTransaction().commit();
	...
	try {
		...
		throw new NonexistentEntityException("The continente with id "
			+ id + " no longer exists.", enfe);
	    }
	    List<String> illegalOrphanMessages = null;
	    Collection<Territorio> territorioCollectionOrphanCheck = continente
		    .getTerritorioCollection();
		...
		illegalOrphanMessages
			.add("This Continente ("
				+ continente
				+ ") cannot be destroyed since the Territorio "
				+ territorioCollectionOrphanCheckTerritorio
				+ " in its territorioCollection field has a non-nullable codContinente field.");
	    }
	    if (illegalOrphanMessages != null) {
		throw new IllegalOrphanException(illegalOrphanMessages);
	    }
	    ...
	    Collection<Objconqcont> objconqcontCollection = continente
		    .getObjconqcontCollection();
	    for (Objconqcont objconqcontCollectionObjconqcont : objconqcontCollection) {
		objconqcontCollectionObjconqcont.getContinenteCollection()
			.remove(continente);
		objconqcontCollectionObjconqcont = em
			.merge(objconqcontCollectionObjconqcont);
	    }
    ...
    public List<Continente> findContinenteEntities(int maxResults,
	    int firstResult) {
	return findContinenteEntities(false, maxResults, firstResult);
    }

    private List<Continente> findContinenteEntities(boolean all,
