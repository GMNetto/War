...
public class HistoricoJpaController implements Serializable {
    ...
    public void create(Historico historico) throws IllegalOrphanException,
	...
	if (partidaOrphanCheck != null) {
		...
		illegalOrphanMessages
			.add("The Partida "
				+ partidaOrphanCheck
				+ " already has an item of type Historico whose partida column cannot be null. Please make another selection for the partida field.");
	    }
	}
	if (illegalOrphanMessages != null) {
	...
	try {
		...
		throw new PreexistingEntityException("Historico " + historico
			+ " already exists.", ex);
	    }
	    throw ex;
	} finally {
    ...
    public void edit(Historico historico) throws IllegalOrphanException,
	    NonexistentEntityException, Exception {
	EntityManager em = null;
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Historico persistentHistorico = em.find(Historico.class,
	...
	try {
		...
		throw new NonexistentEntityException("The historico with id "
    ...
    private List<Historico> findHistoricoEntities(boolean all, int maxResults,
	    int firstResult) {
