...
public class ObjterritorioJpaController implements Serializable {
    ...
    public void create(Objterritorio objterritorio)
	...
	if (objetivoOrphanCheck != null) {
	    Objterritorio oldObjterritorioOfObjetivo = objetivoOrphanCheck
		    .getObjterritorio();
		...
		illegalOrphanMessages
			.add("The Objetivo "
				+ objetivoOrphanCheck
				+ " already has an item of type Objterritorio whose objetivo column cannot be null. Please make another selection for the objetivo field.");
	    }
	}
	if (illegalOrphanMessages != null) {
	...
	try {
		...
		throw new PreexistingEntityException("Objterritorio "
			+ objterritorio + " already exists.", ex);
	    }
	    throw ex;
	} finally {
	    if (em != null) {
    ...
    public void edit(Objterritorio objterritorio)
	...
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Objterritorio persistentObjterritorio = em.find(
		    Objterritorio.class, objterritorio.getCodObjetivo());
		...
		Objterritorio oldObjterritorioOfObjetivo = objetivoNew
			.getObjterritorio();
		if (oldObjterritorioOfObjetivo != null) {
		    if (illegalOrphanMessages == null) {
			illegalOrphanMessages = new ArrayList<String>();
    ...
    public List<Objterritorio> findObjterritorioEntities(int maxResults,
	    int firstResult) {
	return findObjterritorioEntities(false, maxResults, firstResult);
    }

    private List<Objterritorio> findObjterritorioEntities(boolean all,
