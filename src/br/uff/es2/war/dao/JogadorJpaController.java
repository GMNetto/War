...
public class JogadorJpaController implements Serializable {
	...
	try {
	    ...
	    for (Jogam jogamCollectionJogamToAttach : jogador
		    .getJogamCollection()) {
	    ...
	    for (Historico historicoCollectionHistoricoToAttach : jogador
	    ...
	    for (Partida partidaCollectionPartidaToAttach : jogador
		    .getPartidaCollection()) {
		partidaCollectionPartidaToAttach = em.getReference(
			partidaCollectionPartidaToAttach.getClass(),
			partidaCollectionPartidaToAttach.getCodPartida());
		...
		Jogador oldJogadorOfJogamCollectionJogam = jogamCollectionJogam
			.getJogador();
		...
		if (oldJogadorOfJogamCollectionJogam != null) {
		    oldJogadorOfJogamCollectionJogam.getJogamCollection()
			    .remove(jogamCollectionJogam);
		    oldJogadorOfJogamCollectionJogam = em
			    .merge(oldJogadorOfJogamCollectionJogam);
		}
	    }
	    for (Historico historicoCollectionHistorico : jogador
		    .getHistoricoCollection()) {
		Jogador oldCodJogadorOfHistoricoCollectionHistorico = historicoCollectionHistorico
			.getCodJogador();
		...
		if (oldCodJogadorOfHistoricoCollectionHistorico != null) {
		    oldCodJogadorOfHistoricoCollectionHistorico
			    .getHistoricoCollection().remove(
				    historicoCollectionHistorico);
		    oldCodJogadorOfHistoricoCollectionHistorico = em
			    .merge(oldCodJogadorOfHistoricoCollectionHistorico);
		}
	    }
	    for (Partida partidaCollectionPartida : jogador
		    .getPartidaCollection()) {
		Jogador oldCodJogadorOfPartidaCollectionPartida = partidaCollectionPartida
			.getCodJogador();
		...
		if (oldCodJogadorOfPartidaCollectionPartida != null) {
		    oldCodJogadorOfPartidaCollectionPartida
			    .getPartidaCollection().remove(
				    partidaCollectionPartida);
		    oldCodJogadorOfPartidaCollectionPartida = em
			    .merge(oldCodJogadorOfPartidaCollectionPartida);
		}
	    }
    ...
    public void edit(Jogador jogador) throws IllegalOrphanException,
	    NonexistentEntityException, Exception {
	EntityManager em = null;
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Jogador persistentJogador = em.find(Jogador.class,
		    jogador.getCodJogador());
	    Collection<Jogam> jogamCollectionOld = persistentJogador
		    .getJogamCollection();
	    Collection<Jogam> jogamCollectionNew = jogador.getJogamCollection();
	    Collection<Historico> historicoCollectionOld = persistentJogador
		    .getHistoricoCollection();
	    Collection<Historico> historicoCollectionNew = jogador
		    .getHistoricoCollection();
	    Collection<Partida> partidaCollectionOld = persistentJogador
		    .getPartidaCollection();
	    Collection<Partida> partidaCollectionNew = jogador
		    .getPartidaCollection();
		...
		if (!jogamCollectionNew.contains(jogamCollectionOldJogam)) {
		    ...
		    illegalOrphanMessages.add("You must retain Jogam "
			    ...
			    + " since its jogador field is not nullable.");
		}
	    }
	    if (illegalOrphanMessages != null) {
		...
		partidaCollectionNewPartidaToAttach = em.getReference(
			partidaCollectionNewPartidaToAttach.getClass(),
			partidaCollectionNewPartidaToAttach.getCodPartida());
		attachedPartidaCollectionNew
			.add(partidaCollectionNewPartidaToAttach);
	    }
		...
		if (!jogamCollectionOld.contains(jogamCollectionNewJogam)) {
		    Jogador oldJogadorOfJogamCollectionNewJogam = jogamCollectionNewJogam
			    .getJogador();
		    jogamCollectionNewJogam.setJogador(jogador);
		    jogamCollectionNewJogam = em.merge(jogamCollectionNewJogam);
		    if (oldJogadorOfJogamCollectionNewJogam != null
			    && !oldJogadorOfJogamCollectionNewJogam
				    .equals(jogador)) {
			oldJogadorOfJogamCollectionNewJogam
				.getJogamCollection().remove(
					jogamCollectionNewJogam);
			oldJogadorOfJogamCollectionNewJogam = em
				.merge(oldJogadorOfJogamCollectionNewJogam);
		    }
		}
	    }
		...
		if (!historicoCollectionOld
		    ...
		    Jogador oldCodJogadorOfHistoricoCollectionNewHistorico = historicoCollectionNewHistorico
			    .getCodJogador();
		    ...
		    if (oldCodJogadorOfHistoricoCollectionNewHistorico != null
			    && !oldCodJogadorOfHistoricoCollectionNewHistorico
				    .equals(jogador)) {
			oldCodJogadorOfHistoricoCollectionNewHistorico
				.getHistoricoCollection().remove(
					historicoCollectionNewHistorico);
			oldCodJogadorOfHistoricoCollectionNewHistorico = em
				.merge(oldCodJogadorOfHistoricoCollectionNewHistorico);
		    }
		}
	    }
		...
		if (!partidaCollectionNew.contains(partidaCollectionOldPartida)) {
		    ...
		    partidaCollectionOldPartida = em
			    .merge(partidaCollectionOldPartida);
		}
	    }
	    for (Partida partidaCollectionNewPartida : partidaCollectionNew) {
		if (!partidaCollectionOld.contains(partidaCollectionNewPartida)) {
		    Jogador oldCodJogadorOfPartidaCollectionNewPartida = partidaCollectionNewPartida
			    .getCodJogador();
		    partidaCollectionNewPartida.setCodJogador(jogador);
		    partidaCollectionNewPartida = em
			    .merge(partidaCollectionNewPartida);
		    if (oldCodJogadorOfPartidaCollectionNewPartida != null
			    && !oldCodJogadorOfPartidaCollectionNewPartida
				    .equals(jogador)) {
			oldCodJogadorOfPartidaCollectionNewPartida
				.getPartidaCollection().remove(
					partidaCollectionNewPartida);
			oldCodJogadorOfPartidaCollectionNewPartida = em
				.merge(oldCodJogadorOfPartidaCollectionNewPartida);
		    }
		}
	    }
	    em.getTransaction().commit();
		...
		if (findJogador(id) == null) {
		    throw new NonexistentEntityException("The jogador with id "
			    + id + " no longer exists.");
		}
	    }
	    throw ex;
	...
	try {
	    ...
	    List<String> illegalOrphanMessages = null;
	    Collection<Jogam> jogamCollectionOrphanCheck = jogador
		    .getJogamCollection();
		...
		illegalOrphanMessages
			.add("This Jogador ("
				...
				+ " in its jogamCollection field has a non-nullable jogador field.");
	    }
	    if (illegalOrphanMessages != null) {
		throw new IllegalOrphanException(illegalOrphanMessages);
	    }
	    Collection<Historico> historicoCollection = jogador
		    .getHistoricoCollection();
	    ...
	    Collection<Partida> partidaCollection = jogador
		    .getPartidaCollection();
    ...
    private List<Jogador> findJogadorEntities(boolean all, int maxResults,
	    int firstResult) {
