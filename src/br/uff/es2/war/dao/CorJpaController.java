...
public class CorJpaController implements Serializable {
	...
	try {
	    ...
	    for (Objderjogador objderjogadorCollectionObjderjogadorToAttach : cor
		...
		Cor oldCodCorOfJogamCollectionJogam = jogamCollectionJogam
			.getCodCor();
		...
		if (oldCodCorOfJogamCollectionJogam != null) {
		    oldCodCorOfJogamCollectionJogam.getJogamCollection()
			    .remove(jogamCollectionJogam);
		    oldCodCorOfJogamCollectionJogam = em
			    .merge(oldCodCorOfJogamCollectionJogam);
		}
	    }
	    for (Objderjogador objderjogadorCollectionObjderjogador : cor
		    .getObjderjogadorCollection()) {
		Cor oldCorOfObjderjogadorCollectionObjderjogador = objderjogadorCollectionObjderjogador
			.getCor();
		...
		if (oldCorOfObjderjogadorCollectionObjderjogador != null) {
		    oldCorOfObjderjogadorCollectionObjderjogador
			    .getObjderjogadorCollection().remove(
				    objderjogadorCollectionObjderjogador);
		    oldCorOfObjderjogadorCollectionObjderjogador = em
			    .merge(oldCorOfObjderjogadorCollectionObjderjogador);
		}
	    }
    ...
    public void edit(Cor cor) throws IllegalOrphanException,
	...
	try {
	    ...
	    Collection<Jogam> jogamCollectionOld = persistentCor
		    .getJogamCollection();
	    Collection<Jogam> jogamCollectionNew = cor.getJogamCollection();
	    Collection<Objderjogador> objderjogadorCollectionOld = persistentCor
		    .getObjderjogadorCollection();
	    Collection<Objderjogador> objderjogadorCollectionNew = cor
		    .getObjderjogadorCollection();
		...
		if (!jogamCollectionNew.contains(jogamCollectionOldJogam)) {
		    ...
		    illegalOrphanMessages.add("You must retain Jogam "
			    ...
			    + " since its codCor field is not nullable.");
		}
	    }
		...
		if (!objderjogadorCollectionNew
		    ...
		    illegalOrphanMessages.add("You must retain Objderjogador "
			    ...
			    + " since its cor field is not nullable.");
		}
	    }
	    if (illegalOrphanMessages != null) {
		...
		if (!jogamCollectionOld.contains(jogamCollectionNewJogam)) {
		    Cor oldCodCorOfJogamCollectionNewJogam = jogamCollectionNewJogam
			    .getCodCor();
		    jogamCollectionNewJogam.setCodCor(cor);
		    jogamCollectionNewJogam = em.merge(jogamCollectionNewJogam);
		    if (oldCodCorOfJogamCollectionNewJogam != null
			    && !oldCodCorOfJogamCollectionNewJogam.equals(cor)) {
			oldCodCorOfJogamCollectionNewJogam.getJogamCollection()
				.remove(jogamCollectionNewJogam);
			oldCodCorOfJogamCollectionNewJogam = em
				.merge(oldCodCorOfJogamCollectionNewJogam);
		    }
		}
	    }
		...
		if (!objderjogadorCollectionOld
		    ...
		    Cor oldCorOfObjderjogadorCollectionNewObjderjogador = objderjogadorCollectionNewObjderjogador
			    .getCor();
		    ...
		    if (oldCorOfObjderjogadorCollectionNewObjderjogador != null
			    && !oldCorOfObjderjogadorCollectionNewObjderjogador
				    .equals(cor)) {
			oldCorOfObjderjogadorCollectionNewObjderjogador
				.getObjderjogadorCollection()
				.remove(objderjogadorCollectionNewObjderjogador);
			oldCorOfObjderjogadorCollectionNewObjderjogador = em
				.merge(oldCorOfObjderjogadorCollectionNewObjderjogador);
		    }
		}
	    }
	    em.getTransaction().commit();
		...
		if (findCor(id) == null) {
		    throw new NonexistentEntityException("The cor with id "
			    + id + " no longer exists.");
		}
	    }
	    throw ex;
	...
	try {
		...
		throw new NonexistentEntityException("The cor with id " + id
			+ " no longer exists.", enfe);
	    }
	    List<String> illegalOrphanMessages = null;
	    Collection<Jogam> jogamCollectionOrphanCheck = cor
		    .getJogamCollection();
		...
		illegalOrphanMessages
			.add("This Cor ("
				...
				+ " in its jogamCollection field has a non-nullable codCor field.");
	    }
	    Collection<Objderjogador> objderjogadorCollectionOrphanCheck = cor
		    .getObjderjogadorCollection();
		...
		illegalOrphanMessages
			.add("This Cor ("
				...
				+ " in its objderjogadorCollection field has a non-nullable cor field.");
	    }
	    if (illegalOrphanMessages != null) {
		throw new IllegalOrphanException(illegalOrphanMessages);
	    }
    ...
    private List<Cor> findCorEntities(boolean all, int maxResults,
	    int firstResult) {
