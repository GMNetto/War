...
public class TerritorioJpaController implements Serializable {
    ...
    public void create(Territorio territorio) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            for (Territorio territorioCollectionTerritorioToAttach : territorio.getTerritorioCollection()) {
            ...
            for (Territorio territorioCollection1TerritorioToAttach : territorio.getTerritorioCollection1()) {
                territorioCollection1TerritorioToAttach = em.getReference(territorioCollection1TerritorioToAttach.getClass(), territorioCollection1TerritorioToAttach.getCodTerritorio());
                attachedTerritorioCollection1.add(territorioCollection1TerritorioToAttach);
            }
            ...
            for (Ocupacao ocupacaoCollectionOcupacaoToAttach : territorio.getOcupacaoCollection()) {
            ...
            for (Territorio territorioCollectionTerritorio : territorio.getTerritorioCollection()) {
                territorioCollectionTerritorio.getTerritorioCollection().add(territorio);
                territorioCollectionTerritorio = em.merge(territorioCollectionTerritorio);
            }
            for (Territorio territorioCollection1Territorio : territorio.getTerritorioCollection1()) {
                territorioCollection1Territorio.getTerritorioCollection().add(territorio);
                territorioCollection1Territorio = em.merge(territorioCollection1Territorio);
            }
            for (Ocupacao ocupacaoCollectionOcupacao : territorio.getOcupacaoCollection()) {
                Territorio oldTerritorioOfOcupacaoCollectionOcupacao = ocupacaoCollectionOcupacao.getTerritorio();
                ...
                if (oldTerritorioOfOcupacaoCollectionOcupacao != null) {
                    oldTerritorioOfOcupacaoCollectionOcupacao.getOcupacaoCollection().remove(ocupacaoCollectionOcupacao);
                    oldTerritorioOfOcupacaoCollectionOcupacao = em.merge(oldTerritorioOfOcupacaoCollectionOcupacao);
                }
            }
    ...
    public void edit(Territorio territorio) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Territorio persistentTerritorio = em.find(Territorio.class, territorio.getCodTerritorio());
            Carta cartaOld = persistentTerritorio.getCarta();
            Carta cartaNew = territorio.getCarta();
            Continente codContinenteOld = persistentTerritorio.getCodContinente();
            Continente codContinenteNew = territorio.getCodContinente();
            Collection<Territorio> territorioCollectionOld = persistentTerritorio.getTerritorioCollection();
            Collection<Territorio> territorioCollectionNew = territorio.getTerritorioCollection();
            Collection<Territorio> territorioCollection1Old = persistentTerritorio.getTerritorioCollection1();
            Collection<Territorio> territorioCollection1New = territorio.getTerritorioCollection1();
            Collection<Ocupacao> ocupacaoCollectionOld = persistentTerritorio.getOcupacaoCollection();
            Collection<Ocupacao> ocupacaoCollectionNew = territorio.getOcupacaoCollection();
            ...
            for (Ocupacao ocupacaoCollectionOldOcupacao : ocupacaoCollectionOld) {
                if (!ocupacaoCollectionNew.contains(ocupacaoCollectionOldOcupacao)) {
                    ...
                    illegalOrphanMessages.add("You must retain Ocupacao " + ocupacaoCollectionOldOcupacao + " since its territorio field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Territorio territorioCollection1NewTerritorioToAttach : territorioCollection1New) {
                territorioCollection1NewTerritorioToAttach = em.getReference(territorioCollection1NewTerritorioToAttach.getClass(), territorioCollection1NewTerritorioToAttach.getCodTerritorio());
                attachedTerritorioCollection1New.add(territorioCollection1NewTerritorioToAttach);
            }
            ...
            if (cartaNew != null && !cartaNew.equals(cartaOld)) {
                Territorio oldCodTerritorioOfCarta = cartaNew.getCodTerritorio();
                if (oldCodTerritorioOfCarta != null) {
                    oldCodTerritorioOfCarta.setCarta(null);
                    oldCodTerritorioOfCarta = em.merge(oldCodTerritorioOfCarta);
                }
            ...
            for (Territorio territorioCollectionOldTerritorio : territorioCollectionOld) {
                if (!territorioCollectionNew.contains(territorioCollectionOldTerritorio)) {
                    territorioCollectionOldTerritorio.getTerritorioCollection().remove(territorio);
                    territorioCollectionOldTerritorio = em.merge(territorioCollectionOldTerritorio);
                }
            }
            for (Territorio territorioCollectionNewTerritorio : territorioCollectionNew) {
                if (!territorioCollectionOld.contains(territorioCollectionNewTerritorio)) {
                    territorioCollectionNewTerritorio.getTerritorioCollection().add(territorio);
            ...
            for (Territorio territorioCollection1OldTerritorio : territorioCollection1Old) {
                if (!territorioCollection1New.contains(territorioCollection1OldTerritorio)) {
                    territorioCollection1OldTerritorio.getTerritorioCollection().remove(territorio);
                    territorioCollection1OldTerritorio = em.merge(territorioCollection1OldTerritorio);
                }
            }
            for (Territorio territorioCollection1NewTerritorio : territorioCollection1New) {
                if (!territorioCollection1Old.contains(territorioCollection1NewTerritorio)) {
                    territorioCollection1NewTerritorio.getTerritorioCollection().add(territorio);
                    territorioCollection1NewTerritorio = em.merge(territorioCollection1NewTerritorio);
                }
            }
            for (Ocupacao ocupacaoCollectionNewOcupacao : ocupacaoCollectionNew) {
                if (!ocupacaoCollectionOld.contains(ocupacaoCollectionNewOcupacao)) {
                    Territorio oldTerritorioOfOcupacaoCollectionNewOcupacao = ocupacaoCollectionNewOcupacao.getTerritorio();
                    ...
                    if (oldTerritorioOfOcupacaoCollectionNewOcupacao != null && !oldTerritorioOfOcupacaoCollectionNewOcupacao.equals(territorio)) {
                        oldTerritorioOfOcupacaoCollectionNewOcupacao.getOcupacaoCollection().remove(ocupacaoCollectionNewOcupacao);
                        oldTerritorioOfOcupacaoCollectionNewOcupacao = em.merge(oldTerritorioOfOcupacaoCollectionNewOcupacao);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            Collection<Ocupacao> ocupacaoCollectionOrphanCheck = territorio.getOcupacaoCollection();
            for (Ocupacao ocupacaoCollectionOrphanCheckOcupacao : ocupacaoCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Territorio (" + territorio + ") cannot be destroyed since the Ocupacao " + ocupacaoCollectionOrphanCheckOcupacao + " in its ocupacaoCollection field has a non-nullable territorio field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
            ...
            Collection<Territorio> territorioCollection = territorio.getTerritorioCollection();
            for (Territorio territorioCollectionTerritorio : territorioCollection) {
                territorioCollectionTerritorio.getTerritorioCollection().remove(territorio);
                territorioCollectionTerritorio = em.merge(territorioCollectionTerritorio);
            }
            Collection<Territorio> territorioCollection1 = territorio.getTerritorioCollection1();
            for (Territorio territorioCollection1Territorio : territorioCollection1) {
                territorioCollection1Territorio.getTerritorioCollection().remove(territorio);
                territorioCollection1Territorio = em.merge(territorioCollection1Territorio);
            }
