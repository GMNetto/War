...
public class PartidaJpaController implements Serializable {
    ...
    public void create(Partida partida) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            if (historico != null) {
                historico = em.getReference(historico.getClass(), historico.getCodPartida());
            ...
            for (Jogam jogamCollectionJogamToAttach : partida.getJogamCollection()) {
            ...
            for (Jogam jogamCollectionJogam : partida.getJogamCollection()) {
                Partida oldPartidaOfJogamCollectionJogam = jogamCollectionJogam.getPartida();
                ...
                if (oldPartidaOfJogamCollectionJogam != null) {
                    oldPartidaOfJogamCollectionJogam.getJogamCollection().remove(jogamCollectionJogam);
                    oldPartidaOfJogamCollectionJogam = em.merge(oldPartidaOfJogamCollectionJogam);
                }
            }
    ...
    public void edit(Partida partida) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Partida persistentPartida = em.find(Partida.class, partida.getCodPartida());
            ...
            Collection<Jogam> jogamCollectionOld = persistentPartida.getJogamCollection();
            ...
            if (historicoOld != null && !historicoOld.equals(historicoNew)) {
                ...
                illegalOrphanMessages.add("You must retain Historico " + historicoOld + " since its partida field is not nullable.");
            }
            for (Jogam jogamCollectionOldJogam : jogamCollectionOld) {
                if (!jogamCollectionNew.contains(jogamCollectionOldJogam)) {
                    ...
                    illegalOrphanMessages.add("You must retain Jogam " + jogamCollectionOldJogam + " since its partida field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            if (historicoNew != null) {
                historicoNew = em.getReference(historicoNew.getClass(), historicoNew.getCodPartida());
            ...
            for (Jogam jogamCollectionNewJogam : jogamCollectionNew) {
                if (!jogamCollectionOld.contains(jogamCollectionNewJogam)) {
                    Partida oldPartidaOfJogamCollectionNewJogam = jogamCollectionNewJogam.getPartida();
                    jogamCollectionNewJogam.setPartida(partida);
                    jogamCollectionNewJogam = em.merge(jogamCollectionNewJogam);
                    if (oldPartidaOfJogamCollectionNewJogam != null && !oldPartidaOfJogamCollectionNewJogam.equals(partida)) {
                        oldPartidaOfJogamCollectionNewJogam.getJogamCollection().remove(jogamCollectionNewJogam);
                        oldPartidaOfJogamCollectionNewJogam = em.merge(oldPartidaOfJogamCollectionNewJogam);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            if (historicoOrphanCheck != null) {
                ...
                illegalOrphanMessages.add("This Partida (" + partida + ") cannot be destroyed since the Historico " + historicoOrphanCheck + " in its historico field has a non-nullable partida field.");
            }
            Collection<Jogam> jogamCollectionOrphanCheck = partida.getJogamCollection();
            for (Jogam jogamCollectionOrphanCheckJogam : jogamCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Partida (" + partida + ") cannot be destroyed since the Jogam " + jogamCollectionOrphanCheckJogam + " in its jogamCollection field has a non-nullable partida field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
