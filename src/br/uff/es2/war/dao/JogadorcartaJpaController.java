...
public class JogadorcartaJpaController implements Serializable {
    ...
    public void create(Jogadorcarta jogadorcarta)
	    throws PreexistingEntityException, Exception {
	if (jogadorcarta.getJogadorcartaPK() == null) {
	    jogadorcarta.setJogadorcartaPK(new JogadorcartaPK());
	}
	jogadorcarta.getJogadorcartaPK().setCodPartida(
		jogadorcarta.getJogam().getJogamPK().getCodPartida());
	jogadorcarta.getJogadorcartaPK().setCodCarta(
		jogadorcarta.getCarta().getCodCarta());
	jogadorcarta.getJogadorcartaPK().setCodJogador(
		jogadorcarta.getJogam().getJogamPK().getCodJogador());
	...
	try {
		...
		throw new PreexistingEntityException("Jogadorcarta "
			+ jogadorcarta + " already exists.", ex);
	    }
	    throw ex;
	} finally {
	    if (em != null) {
    ...
    public void edit(Jogadorcarta jogadorcarta)
	...
	try {
	    em = getEntityManager();
	    em.getTransaction().begin();
	    Jogadorcarta persistentJogadorcarta = em.find(Jogadorcarta.class,
	...
	try {
		...
		throw new NonexistentEntityException(
			"The jogadorcarta with id " + id + " no longer exists.",
			enfe);
	    }
	    Carta carta = jogadorcarta.getCarta();
	    if (carta != null) {
    ...
    public List<Jogadorcarta> findJogadorcartaEntities(int maxResults,
	    int firstResult) {
	return findJogadorcartaEntities(false, maxResults, firstResult);
    }

    private List<Jogadorcarta> findJogadorcartaEntities(boolean all,
