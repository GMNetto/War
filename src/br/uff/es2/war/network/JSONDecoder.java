...
class JSONDecoder implements Decoder {
	...
	Set<Continent> continents = decodeContinents(json.getJSONArray("continents"), world);
	...
	continent.addAll(decodeTerritories(json.getJSONArray("territories"), continent));
	return continent;
    }
    ...
    private Set<Territory> decodeTerritories(JSONArray array, Continent continent) {
	Set<Territory> territories = new HashSet<>();
	for (int i = 0; i < array.length(); i++) {
	    Territory territory = decodeTerritory(array.getJSONObject(i), continent);
	...
	JSONObject json = new JSONObject(code);
	Territory attacker = decodeTerritory(json.getJSONObject("attackingTerritory"), null);
	Territory defender = decodeTerritory(json.getJSONObject("defendingTerritory"), null);
	...
	Territory territory = decodeTerritory(json.getJSONObject("territory"), null);
