...
public class AlocaController {
    ...
    public AlocaController(Pane pane_aloca, Button btn_aloca_mais,
	    Button btn_aloca_cancel, Button btn_aloca_menos,
	    Button btn_aloca_ok, int raio, JogoController jc) {
	...
	this.btn_aloca_mais.setOnAction(new EventHandler<ActionEvent>() {
		...
		if (acrescenta < maxExercitos) {
		    acrescenta++;
		    terDestino.getTexto().setText(
			    (terDestino.getQtd() + acrescenta) + "");
		}
	    }
	});
	...
	this.btn_aloca_mais.setOnAction(new EventHandler<ActionEvent>() {
	    ...
	    public void handle(ActionEvent event) {
		// limitando para numero máximo de execitos a serem alocados
		// como colocar a condição de não mover exercito que já foram
		// movidos nessa fase
		if (acrescenta <= terOrigem.getQtd() - 1) {
		    acrescenta++;
		    terDestino.getTexto().setText(
			    (terDestino.getQtd() + acrescenta) + "");
		    terOrigem.getTexto().setText(
			    (terOrigem.getQtd() - acrescenta) + "");
		}
	    }
	});
